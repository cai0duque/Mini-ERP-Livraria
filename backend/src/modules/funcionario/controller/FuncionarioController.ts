...
export const FuncionarioController = {
  ...
  remove: async (req: Request, res: Response) => {
    try {
      ...
      if (e?.code === "P2025") return res.status(404).json({ error: "Funcionário não encontrado" });
      if (e?.code === "P2003") return res.status(400).json({ error: "Há compras vinculadas a este funcionário" });
      res.status(400).json({ error: e?.message || "Erro ao remover funcionário" });
    }
  },
};
